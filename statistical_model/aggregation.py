            ...
            for policy in os.listdir(user_path):
                ...
                for storage in os.listdir(policy_path):
                    ...
                    if storage == 'cookies':
                        ...
                        # Other storages: load consolidated
                        analysis_json = os.path.join(storage_path, 'consolidated', 'analysis.json')
                        if os.path.exists(analysis_json):
                            try:
                                with open(analysis_json, 'r') as f:
                                    data = json.load(f)
...
def aggregate_by_dimension(files: List[Dict[str, Any]], 
    ...
    for key, group_files in grouped.items():
        ...
        if storage == 'cookies':
            # For cookies, only aggregate 'added' lifecycle
            cookie_added = [f for f in group_files if f['lifecycle'] == 'added']
            if cookie_added:
                aggregated[key] = aggregate_cookie_data(cookie_added)
        else:
            aggregated[key] = aggregate_storage_data(group_files)
...
def create_aggregations(base_path: str, output_path: str):
    ...
    try:
        from statistical_model.aggregation_viz import create_aggregation_visualizations
