...
def calculate_entropy(value: str) -> float:
    ...
    # Compter la fréquence de chaque caractre
...
def decode_value(value: str) -> Tuple[str, str, bool]:
    ...
    Returns:
        Tuple (valeur_décodée, méthode_décodage, succs)
        - valeur_décodée: La valeur aprs décodage
        - méthode_décodage: 'base64', 'url', 'json', 'jwt', 'none'
        - succs: True si décodage réussi
    """
    if not value:
    ...
    # Tentative 1: Base64
    try:
        # Vérifier si a ressemble  du Base64
    ...
    if value.count('.') == 2:
        try:
            parts = value.split('.')
            # Décoder la partie payload (2me partie)
            payload = parts[1]
...
def detect_data_type(value: str, decoded_value: str = None) -> str:
    ...
    # Token/ID long (20+ caractres alphanumériques)
    ...
    # Texte court (< 20 caractres, pas de caractres spéciaux)
...
def extract_vendor_from_domain(domain: str) -> str:
    """
    Extrait le vendor (entreprise)  partir d'un domaine.
...
def analyze_cookie_privacy(cookie: Dict) -> Dict:
    """
    Analyse complte vie privée d'un cookie.
