...
Génre des visualisations spécifiques pour chaque catégorie de données personnelles.
"""
...
def calculate_lifetime_category(expires: float, collection_timestamp: float = None) -> str:
    ...
    # Si la durée est négative (cookie déj expiré au moment de la collecte), considérer comme Session
...
def is_third_party(cookie_domain: str, initial_url: str) -> bool:
    ...
    try:
        ...
        # Vérifier si le domaine du cookie correspond  l'URL
...
def analyze_category(cookies: List[Dict], category_name: str) -> Dict:
    ...
    for cookie in cookies:
        ...
        # Critres : PII (toujours vrai) + Durée >1 an + HttpOnly=False + Third-Party
        risk_score = 0
        
        # Critre 1: Durée > 1 an (365 jours)
        ...
        # Critre 2: HttpOnly=False
        if not http_only:
            risk_score += 1
        
        # Critre 3: Third-Party
        if is_tp:
            risk_score += 1
...
def generate_category_visualizations(analysis_results: Dict, output_dir: Path, viz):
    """Génre les 11 visualisations standards pour une catégorie"""
...
def generate_category_report(analysis_results: Dict, output_path: Path):
    """Génre un rapport markdown pour une catégorie"""
    category_name = analysis_results['category_name']
...
** Constat** : {lifetime_long} cookies ({lifetime_long/total*100:.1f}%) ont une durée de vie  6 mois.
...
def main():
    ...
    print("=" * 70)
    print(" Analyse par catégorie terminée avec succs!")
    print("=" * 70)
