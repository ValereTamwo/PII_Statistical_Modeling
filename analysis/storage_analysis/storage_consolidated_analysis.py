...
def load_all_storage_items(input_dir: Path, storage_type: str) -> Tuple[List[Dict], List[Dict]]:
    ...
    for category_file in category_files:
        ...
        # Ajouter le nom de catégorie et le type de stockage  chaque item
        for item in items:
...
def analyze_storage_consolidated(direct_pii_items: List[Dict], other_items: List[Dict], 
    ...
    for item in all_items:
        ...
        # Vendor extraction selon le type de stockage
        if storage_type == 'indexeddb':
            # IndexedDB : extraire depuis source_file
            source_file = item.get('source_file', '')
            if source_file:
                # Ex: "https_www.amoursucre.com_0.indexeddb.leveldb.json" -> "www.amoursucre.com"
            ...
            # localStorage et sessionStorage : extraire depuis initial_url
            initial_url = item.get('initial_url', '')
            if initial_url:
                # Ex: "https://www.passagedudesir.fr" -> "www.passagedudesir.fr"
...
def main():
    ...
    print("\n" + "=" * 70)
    print(" Analyse consolidée des stockages terminée avec succs!")
    print("=" * 70)
