...
Combine added et modified pour raconter l'histoire complte de chaque cookie.

Génre 5 graphiques temporels :
...
def build_cookie_timeline(cookie_key: str, added_cookies: List[Dict], modified_cookies: List[Dict], removed_cookies: List[Dict] = None) -> Dict:
    """
    Construit la timeline complte d'un cookie.
...
def analyze_lifecycle(added_dir: Path, modified_dir: Path, removed_dir: Path = None) -> Dict:
    """
    Analyse complte du cycle de vie des cookies.
    """
    print("\n Analyse de Cycle de Vie des Cookies")
    print("=" * 70)
        ...
        for auth_status in auth_statuses:
            for policy in policies:
                ...
                if not added_dir.exists() and not modified_dir.exists():
                    print(f"Le dossier {added_dir} n'existe pas, passage  la configuration suivante.")
                    continue
                ...
                # Convertir pour JSON (exclure les timelines compltes, trop volumineuses)
                ...
                # Sauvegarder les timelines compltes (fichier séparé)
                print("\n Sauvegarde des timelines compltes...")
                ...
                print(f"    {len(timelines_serializable)} timelines compltes sauvegardées")
