"""
Consolidated Analysis Script - Strategic Approach:
- DIRECT_PII: Granular analysis by sub-category (email, gender, etc.)
- Secondary Categories: Behavioral and Identity tracking analysis
"""
...
def load_all_cookies(input_dir: Path) -> Tuple[List[Dict], List[Dict]]:
    ...
    for category_file in category_files:
        ...
        # Split by direct PII classification
...
def analyze_consolidated(direct_pii_cookies: List[Dict], other_cookies: List[Dict]) -> Dict:
    ...
    for cookie in all_cookies:
        ...
        # Calculer duration_days  partir du champ 'expires' si disponible (supporte timestamp et ISO/str)
...
def generate_all_visualizations(analysis_results: Dict, output_dir: Path):
    """Generates consolidated visualization suite (requires visualizations module)"""
    ...
    print(f"\n 22 graphiques consolidés générés dans {output_dir / 'consolidated'}")


def main():
    """Main execution script for consolidated analysis."""
        ...
        for auth_status in auth_statuses:
            for policy in policies:
                ...
                for lifecycle in ['added', 'modified', 'removed']:
                    ...
                    # Lifecycle mapping (removed -> deleted)
